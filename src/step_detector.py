...
BEST_MODEL_PATH = Path("data/runs/train/exp/weights/best.pt")
ANNOTATED_IMGS_DIR = Path(
    "data/runs/validate/test_evaluation/annotated_images"
)
...
def save_annotated_images(model, data_yaml_path, output_dir):
    """
    ...
    for img_path in test_images:
        ...
        # Plot ground truth boxes (if available)
        label_path = (
        ...
        # Add legend and save
        plt.legend()
        plt.title(f"Predictions vs Ground Truth: {Path(img_path).name}")
        plt.axis("off")
...
def evaluate_model(
    ...
    try:
        ...
        if should_save_annotated_images:
            save_annotated_images(model, data_yaml_path, ANNOTATED_IMGS_DIR)

        # Log detailed metrics
        logger.info(f"mAP50: {results.box.map50:.4f}")  # mAP at IoU 0.50
        logger.info(f"mAP50-95: {results.box.map:.4f}")  # mAP at IoU 0.50:0.95
...
def main(train=False, evaluate=False, test=False):
    ...
    if train:
        # Start training
        setup_yolo_training(data_yaml_path=ANNOTATIONS_PATH)
    ...
    if test:
        try:
            ...
            test_model(data_yaml_path=ANNOTATIONS_PATH)
        except Exception as e:
            logger.error(f"An error occurred during testing: {str(e)}")
