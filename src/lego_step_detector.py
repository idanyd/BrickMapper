        ...
        for epoch in range(num_epochs):
            ...
            for images, targets in train_loader:
                ...
                if batch_count % 10 == 0:  # Log every 10 batches
                    logger.debug(
                        f"Epoch {epoch + 1}, Batch {batch_count}: Loss = {losses.item():.4f}"
                    )
        ...
        for step_info in steps.values():
            if "bbox" in step_info:
                ...
                if "number_bbox" in step_info:
                    ...
                    if number_region:
                        step_dict["step_number"] = self._process_number(
                            number_region
                        )
...
def train_model(images_dir: Path, model_save_path: Path, num_epochs: int = 10):
