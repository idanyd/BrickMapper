...
const BUILDING_INSTRUCTIONS_URL =
  "https://www.lego.com/en-gb/service/building-instructions";
const BUILDING_INSTRUCTIONS_PDFS_URL =
    ...
    <div className="p-6">
      <div className="max-w-4xl p-6 bg-white rounded shadow">
        ...
        <p className="text-gray-700 text-lg">
          BrickMapper is a web application that helps LEGO® enthusiasts identify
          and locate specific LEGO® pieces within instruction manuals.
          <br />
          To find which steps a LEGO® piece appears in across different
          instruction booklets, enter the element ID of the piece as it appears
          in the set's inventory pages.
          <br />
          You can also upload an image of a LEGO® piece, and BrickMapper will
          use the{" "}
          <a
            href="https://www.brickognize.com"
          ...
          API to recognize the piece and provide you with its element ID.
          <br />
          The BrickMapper database was created using information extracted from
          official LEGO® instruction manuals and from the{" "}
          <a
          ...
          . This data is used solely to help users identify and locate LEGO®
          elements within instruction manuals.
        </p>
      </div>
    </div>
  );
      ...
      <div className="max-w-xl p-6 bg-white rounded shadow">
        ...
        <div className="space-y-6 text-gray-700">
          <div>
            ...
            <p>
              By accessing and using BrickMapper, you agree to be bound by these
          ...
          <div>
            ...
            <p>
              BrickMapper is intended for personal, non-commercial use. You may
              use the website to identify and locate LEGO® pieces within
              instruction manuals.
            </p>
          </div>
          ...
          <div>
            ...
            <p>
              The content and materials on BrickMapper, including but not
              limited to text, graphics, logos, and software, are owned by
              BrickMapper or its licensors and are protected by copyright and
          ...
          <div>
            ...
            <p>
              BrickMapper is provided "as is" without any warranties, express or
              implied. We do not warrant that the website will be error-free or
              uninterrupted, or that the information provided will be accurate
              or complete.
            </p>
          </div>
          ...
          <div>
            ...
            <p>
              In no event shall BrickMapper be liable for any direct, indirect,
              incidental, special, or consequential damages arising out of or in
              any way connected with your use of the website.
            </p>
          </div>
          ...
          <div>
            ...
            <p>
              BrickMapper may contain links to third-party websites. We are not
          ...
          <div>
            ...
            <p>
              ...
              Your continued use of BrickMapper after any such changes
          ...
          <div>
            ...
            <p>
              BrickMapper does not gather any information about the users or
              their behaviour when using the site.
            </p>
            <p>
              However, images uploaded by the user are sent to Brickognize to be
              analyzed, and by using that option you're agreeing to
              Brickognize's{" "}
              <a
          ...
          <div>
            ...
            <p>
              BrickMapper is an independent project and is not affiliated with,
              endorsed by, or associated with the LEGO® Group, Brickognize, or
              Rebrickable® in any way.
            </p>
          </div>
...
const Home = () => {
  ...
  // Helper function to parse set filter
  const parseSetFilter = (filterString) => {
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Brickognize API error: ${response.status} ${response.statusText}`
        );
      }
      ...
      if (result.items && result.items.length > 0) {
        ...
        alert(
          "No LEGO parts were recognized in this image. Please try a different image."
        );
      }
    } catch (error) {
  ...
  const selectBrickognizePart = async (selectedPart) => {
    ...
    try {
      ...
      if (!matchingElements || matchingElements.length === 0) {
        alert(
          `Part ${selectedPart.id} (${selectedPart.name}) was recognized but is not in our database.`
        );
        return;
      }
  ...
  const searchStepsForElement = async (elementIdToSearch) => {
    try {
      ...
      // Parse set filter
      const allowedSets = parseSetFilter(setFilter);
      console.log("Set filter:", allowedSets);
      ...
      if (allowedSets.length > 0) {
        // Create an array of patterns for set numbers (handle both with and without suffixes)
        const setPatterns = allowedSets.flatMap((setNum) => [
          setNum, // Exact match
          `${setNum}-%`, // Pattern for sets with suffixes like "10294-1"
        ]);

        // Use OR condition to match any of the allowed sets
        const setConditions = allowedSets
          .map((setNum) => `set_num.eq.${setNum},set_num.like.${setNum}-%`)
          .join(",");

        stepsQuery = stepsQuery.or(setConditions);
      }
      ...
      if (!stepsData || stepsData.length === 0) {
        const filterMessage =
          allowedSets.length > 0
            ? ` in the specified sets (${allowedSets.join(", ")})`
            : "";
        alert(
          `Element found but no steps contain this element${filterMessage}.`
        );
        setSearchResults([]);
        return;
      }
      ...
      const setBookletPairs = [
        ...new Set(
          stepsData.map((step) => `${step.set_num}|${step.booklet_number}`)
        ),
      ].map((pair) => {
      ...
      const manualPromises = setBookletPairs.map(
        ({ set_num, booklet_number }) =>
          supabase
            .from("manuals")
            .select("manual_id")
            .eq("set_num", set_num)
      ...
      const enrichedStepsData = stepsData.map((step) => {
        const setInfo =
          setsData.find((set) => set.set_num === step.set_num) || {};
    ...
    try {
      ...
      if (exactMatch && exactMatch.length > 0) {
        ...
        if (!partialMatch || partialMatch.length === 0) {
          ...
          alert(
            `Found ${partialMatch.length} matching elements. Please be more specific.`
          );
      ...
      <section className="mb-8">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Filter by Set Numbers (optional)
          </label>
          <input
            type="text"
            value={setFilter}
            onChange={(e) => setSetFilter(e.target.value)}
            placeholder="Enter set numbers separated by commas"
            className="border border-gray-300 rounded px-4 py-2 w-full"
          />
          <p className="text-sm text-gray-500 mt-1">
            Leave empty to search all sets, or enter specific set numbers to
            limit results
          </p>
        </div>
        ...
        {searchMode === "id" ? (
          ...
          <div className="mb-4">
              ...
              <div className="border border-gray-300 rounded p-4 mb-4">
                ...
                {brickognizeResults.length > 6 && (
                  <p className="text-sm text-gray-500 mt-2">
                    Showing top 6 results out of {brickognizeResults.length}{" "}
                    matches
                  </p>
                )}
              </div>
            ...
            {showElementSelection && elementVariants.length > 0 && (
              <div className="border border-gray-300 rounded p-4 mb-4">
                <h3 className="text-lg font-semibold mb-3">
                  Multiple color variants found for "
                  {selectedBrickognizePart?.name}" (Part{" "}
                  {selectedBrickognizePart?.id}). Select a color variant to
                  search for:
                </h3>
          ...
          <div className="p-4 border border-gray-300 rounded mb-4">
            ...
            {setFilter && (
              <p>
                ...
                {parseSetFilter(setFilter).join(", ") || "All sets"}
              </p>
            )}
            <img
    ...
    <Router>
      <div className="container">
        ...
        <footer className="text-center mt-8 py-4 bg-gray-100">
          <p>
            Copyright © {new Date().getFullYear()} Idan Dekel. All rights
            reserved.
          </p>
          <p>
